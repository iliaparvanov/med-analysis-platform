...
def update_subscriptions(hospitals_model):
    hospitals_with_canceled_subs = hospitals_model.objects.filter(subscription__downgrade_at_period_end=True)
    ...
    hospitals_with_overdue_payments = hospitals_model.objects.filter(subscription__current_period_end__lte=date.today() + relativedelta(days=-7))
...
def start(hospitals_model):
    scheduler = BackgroundScheduler()
    scheduler.add_job(update_subscriptions, 'interval', [hospitals_model], minutes=30)
    scheduler.start()
